...
): Promise<PagedData<ApplicationItem>> {
  const res = await http.get<ApiResponse<PagedData<ApplicationItem>>>('/api/applications/my', {
    params,
  });
...
export async function getJobApplications(
  ...
  const res = await http.get<ApiResponse<PagedData<ApplicationItem>>>(
...
export async function reviewApplication(
  ...
  const res = await http.patch<ApiResponse<ApplicationItem>>(
    `/api/employer/applications/${id}/review`,
    { action }
  );
  return res.data.data as ApplicationItem;
}

...
import type { ApiResponse, CmplStatus, ComplaintItem, PagedData } from '@student-side-job-platform/shared-types';
...
export async function getAdminComplaints(params: {
  ...
  const res = await http.get<ApiResponse<PagedData<AdminComplaintItem>>>('/api/admin/complaints', { params });
...
export async function handleAdminComplaint(
  ...
  const res = await http.patch<ApiResponse<AdminComplaintItem>>(`/api/admin/complaints/${id}/handle`, payload);
